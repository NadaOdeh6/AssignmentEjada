...
public class APITests {
	...
	public void testPostOrder() {
		...
		Response response = given().auth().oauth2(accessToken).header("Content-Type", "application/json")
				.body(requestBody).when().post("/orders").then().statusCode(201).body("created", equalTo(true))
				.extract().response();
	...
	public void testDeleteOrder() {
		...
		given().header("Authorization", "Bearer " + accessToken).when().delete("/orders/" + createdOrderId).then()
